...
# Fenton growth charts for male and female weight were digitized up to 40 weeks
# (for full-term) from the literature [1-3] using PinPoint Digitizer [4].
...
# https://doi.org/10.1186/1471-2431-13-92
...
csv0_male <- read.csv(
  "data-raw/fenton-preterm-boys-wtkg-digitized.csv", stringsAsFactors = F
)
csv0_female <- read.csv(
  "data-raw/fenton-preterm-girls-wtkg-digitized.csv", stringsAsFactors = F
)
...
for(iter in seq_len(nrow(lms_listing))){
  ...
  wtkg <- c(
    csv0[
      which(
        csv0$SEXF == lms_listing[iter,"SEXF"] & 
          csv0$TIME_WK == lms_listing[iter,"GA_WK"] & 
          csv0$PTILE == perc[1]
      ),
      "WTKG"
    ],
  ...
  wtkg_val[iter,paste0("PP",ifelse(perc[1] < 10,"0",""),perc[1])] <- fun_pred(
    PERC = perc[1], param = fit$par
  )
  wtkg_val[iter,paste0("PP",ifelse(perc[2] < 10,"0",""),perc[2])] <- fun_pred(
    PERC = perc[2], param = fit$par
  )
  wtkg_val[iter,paste0("PP",ifelse(perc[3] < 10,"0",""),perc[3])] <- fun_pred(
    PERC = perc[3], param = fit$par
  )
  wtkg_val[iter,paste0("PP",ifelse(perc[4] < 10,"0",""),perc[4])] <- fun_pred(
    PERC = perc[4], param = fit$par
  )
  wtkg_val[iter,paste0("PP",ifelse(perc[5] < 10,"0",""),perc[5])] <- fun_pred(
    PERC = perc[5], param = fit$par
  )
...
p1 <- ggplot(wtkg_val %>% filter(SEXF == 0), aes(x = GA_WK))+
  ...
  labs(
    ...
    y = "Percentiles of Weight", 
    title = "Male Fenton Growth Chart", 
    caption = paste0(
      "Note: ", 
      paste0(perc,collapse = ", "),
      " percentiles of weight for age\n
...
p2 <- ggplot(wtkg_val %>% filter(SEXF == 1), aes(x = GA_WK))+
  ...
  labs(
    x = "Gestational Age (week)", 
    y = "Percentiles of Weight", 
    title = "Female Fenton Growth Chart", 
    caption = paste0(
      "Note: ", 
      paste0(perc,collapse = ", "),
      " percentiles of weight for age\n
...
fent0 <- fent_wtage %>%
  ...
  dplyr::mutate(AGEGRP = paste0(
    "PNA: 0; GA_WK: [",
    LOAGEMO,
    ",",
    UPAGEMO,ifelse(LOAGEMO!=UPAGEMO, ")", "]")
  ))%>%
