...
if (!file.exists("data-raw/wtstat.csv")) {
  download.file(
    "https://www.cdc.gov/growthcharts/data/zscore/wtstat.csv", 
    "data-raw/wtstat.csv"
  )
}
...
if (!file.exists("data-raw/wtleninf.csv")) {
  download.file(
    "https://www.cdc.gov/growthcharts/data/zscore/wtleninf.csv", 
    "data-raw/wtleninf.csv"
  )
}
...
if (!file.exists("data-raw/WHO-Boys-Weight-for-length-Percentiles.csv")) {
  download.file(
    ...
    "data-raw/WHO-Boys-Weight-for-length-Percentiles.csv"
  )
}
...
if (!file.exists("data-raw/WHO-Girls-Weight-for-length-Percentiles.csv")) {
  download.file(
    ...
    "data-raw/WHO-Girls-Weight-for-length-Percentiles.csv"
  )
}

## CREATE HTWT0 (MANIPULATED) FROM ORIGINAL CDC AND WHO GROWTH CHARTS ####
...
load_htwt <- function(dir = "data-raw"){
  ...
  who_htwt_male <- read.csv(
    paste0(dir,"/WHO-Boys-Weight-for-length-Percentiles.csv"), 
    stringsAsFactors = F
  )%>%
    dplyr::mutate(SEXF=0)%>%
    dplyr::mutate(VAR="HTWT")
  
  who_htwt_female <- read.csv(
    paste0(dir,"/WHO-Girls-Weight-for-length-Percentiles.csv"), 
    stringsAsFactors = F
  )%>%
    dplyr::mutate(SEXF=1)%>%
    dplyr::mutate(VAR="HTWT")
  ...
  who_htwt <- who_htwt_male %>%
    ...
    dplyr::rename(
      P3=X2nd..2.3rd., P5=X5th, P10=X10th, P25=X25th, P50=X50th, 
