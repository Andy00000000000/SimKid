...
#'
#' @param nseed A positive integer that specifies the number of subjects to simulate. Defaults to 1000.
#' 
...
get_seeds <- function(masterseed = NULL, nseed = 1000){
  ...
  if(length(nseed) != 1L || !is.numeric(nseed) || nseed%%1!=0 || nseed <= 0){warning("nseed must be a positive integer of length one; The default of nseed = 1000 will be used instead")}
  if(length(nseed) != 1L || !is.numeric(nseed) || nseed%%1!=0 || nseed <= 0){nseed <- 1000}
  
  if(is.null(masterseed)){
    seedl <- sample(seq_len(.Machine$integer.max), nseed)
  }else{
    seedl <- withr::with_seed(masterseed, sample(seq_len(.Machine$integer.max), nseed))
  }
}