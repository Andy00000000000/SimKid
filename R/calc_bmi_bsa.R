...
#'
#' @return A data frame with columns of `BMI`, `BSA1`, `BSA2`, and `BSA3` added to `data`:
...
calc_bmi_bsa <- function(data = NULL){
  ...
  data %>%
    dplyr::mutate(BMI = ifelse(!is.na(.data$HTCM), round(.data$WTKG/((.data$HTCM/100)^2),1), NA))%>%
    dplyr::mutate(
      BSA1 = ifelse(!is.na(.data$HTCM), round(sqrt(.data$WTKG*.data$HTCM/3600),2), NA),                 # Mosteller
      BSA2 = ifelse(!is.na(.data$HTCM), round(0.0235*(.data$WTKG^0.51456)*(.data$HTCM^0.42246),2), NA), # Gehan
      BSA3 = ifelse(!is.na(.data$HTCM), round(0.007184*(.data$WTKG^0.425)*(.data$HTCM^0.725),2), NA)    # DuBois
    )
}