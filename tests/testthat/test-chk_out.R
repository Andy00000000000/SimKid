test_that("correct works", {
  demo <- as.data.frame(matrix(0, nrow = 1, ncol = 16))
  colnames(demo) <- c("ID","SEXF","AGEMO","AGE","GAWK","WTKG","HTCM","BMI","BSA1","BSA2","BSA3","ZWTKG","ZHTCM","PWTKG","PHTCM","CHART")
...
test_that("incorrect column names fails", {
  demo <- as.data.frame(matrix(0, nrow = 1, ncol = 16))
  colnames(demo) <- c("ID","SEXF","AGEMO","AGE","GAWK","WTKG","HTCM","BMI","BSA1","BSA2","BSA3","ZWTKG","ZHTCM","PWTKG", "ABC", "CHART")
  expect_error(chk_out(demo = demo, num = 1))
})
